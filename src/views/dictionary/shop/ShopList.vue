...
<div class="content-body">
    ...
    <div class="vld-parent">
        <loading :active.sync="isLoading" :can-cancel="false" @click="onCancel" :is-full-page="fullPage"></loading>
    </div>
    <!-- Content Header -->
    <div class="header-content">
        <div class="toolbar">
            <button class="m-btn m-btn-default" @click="btnAddOnClick">
            ...
            <button class="m-btn m-btn-default" @click="btnEditOnClick">
                <div class="btn-toolbar-icon icon-edit"></div> Sửa
            </button>
            <button class="m-btn m-btn-default" @click="btnDeleteOnClick">
            ...
            <Details @closePopup="closePopup" @hanldeAlert="HandleAlert" :isHide="isHideParent" :titleDialog="title" @reload="reloadData" />
            <DialogDel @closeDelDialog="closePopup" @hanldeAlert="HandleAlert" :isHideDelDialog="isHideDel" :titleDelDialog="title" :shopId="selectedId" :shopName="shopNameSelected" @reload="reloadData" />
        </div>
    </div>
    <!-- Content Body -->
    <div class="grid grid-shop el-table el-table--fit el-table--scrollable-y el-table--enable-row-hover el-table--enable-row-transition">
        <table id="tbListData" cellspacing="0" cellpadding="0" style="min-width: 100%">
            <thead>
                <tr class="el-table__row">
                    <th colspan="1" rowspan="1" width=12% class="el-table_30_column_114 is-leaf">
                        <div class="cell">Mã cửa hàng<br /></div>
                        <div class="group-input">
                            <input class="select-filter-button" placeholder="*" disabled />
                            <input id="txtSearchShopCode" class="input-search" type="text" placeholder="" v-model="txtSearchShopCode" @change="SearchShop()" />
                        </div>

                    </th>
                    <th colspan="1" rowspan="1" width=20% class="el-table_30_column_116 is-leaf">
                        <div class="cell">Tên cửa hàng<br /></div>
                        <div class="group-input">
                            <input class="select-filter-button" placeholder="*" disabled />
                            <input id="txtSearchShopName" class="input-search" type="text" placeholder="" v-model="txtSearchShopName" @change="SearchShop()" />
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" width=44% class="el-table_30_column_115 is-leaf">
                        <div class="cell">Địa chỉ<br /></div>
                        <div class="group-input">
                            <input class="select-filter-button" placeholder="*" disabled />
                            <input id="txtSearchAddress" class="input-search" type="text" placeholder="" v-model="txtSearchAddress" @change="SearchShop()" />
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" width=12% class="el-table_30_column_116 is-leaf">
                        <div class="cell">Số điện thoại<br /></div>
                        <div class="group-input">
                            <input class="select-filter-button" placeholder="*" disabled />
                            <input id="txtSearchPhoneNumber" class="input-search" type="text" placeholder="" v-model="txtSearchPhoneNumber" @change="SearchShop()" />
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" width=12% class="el-table_30_column_116 is-leaf">
                        <div class="cell">Trạng thái<br /></div>
                        <div class="group-input">
                            <select id="txtSearchStatus" class="m-control" v-model="txtSearchStatus" @change="SearchShop()">
                                <option value="">
                                    Tất cả trạng thái
                                </option>
            ...
            <tbody id="tbody">
                <tr class="el-table__row" v-for="shop in shops" :key="shop.shopId" :id="shop.shopId" @click="rowOnClick(shop.shopId, shop.shopName)" @dblclick="rowOnDBClick(shop.shopId)">
                    <td>
    ...
    <div class="paging-bar">
        <div class="paging-option">
            ...
            <div class="m-btn-list-page">
                Trang
                <input type="number" v-model="currentPage" id="current-page" @change="SetCurrentPage()" />
                trên {{totalPage}}
            </div>
